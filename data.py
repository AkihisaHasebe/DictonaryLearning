...
class MVTecDataset:
    def __init__(self, cls : str, size : int = 224, pre_resize : int = 256):
        self.cls = cls
        self.size = size
        if cls in mvtec_classes():
            self._download()
        self.train_ds = MVTecTrainDataset(cls, size, pre_resize)
        self.test_ds = MVTecTestDataset(cls, size, pre_resize)
...
class MVTecTrainDataset(ImageFolder):
    def __init__(self, cls : str, size : int, pre_resize : int):
        super().__init__(
            root=DATASETS_PATH / cls / "train",
            transform=transforms.Compose([
                transforms.Resize(pre_resize, interpolation=transforms.InterpolationMode.BICUBIC),
    ...
    def __init__(self, cls : str, size : int, pre_resize : int):
        super().__init__(
            ...
            target_transform=transforms.Compose([
                transforms.Resize(pre_resize, interpolation=transforms.InterpolationMode.NEAREST
                ),
                transforms.CenterCrop(size),
                transforms.ToTensor(),
            ]),
